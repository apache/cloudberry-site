...
This document introduces how to manually deploy Apache Cloudberry on physical machines using RPM package. Before reading this document, it is recommended to first read the [Software and Hardware Configuration Requirements](/docs/cbdb-op-software-hardware.md) and [Prepare to Deploy Apache Cloudberry on Physical Machine](/docs/cbdb-op-prepare-to-deploy.md).
...
Read the [Prepare to Deploy Apache Cloudberry on Physical Machine](/docs/cbdb-op-prepare-to-deploy.md) document to prepare the server nodes.
...
After you run the `psql` command, the system will prompt you to enter the database password. After you enter the correct password, you will successfully log into the Apache Cloudberry and can perform SQL queries and operations. Make sure that you have the correct permissions to access the target database.
