...
source: https://cloudberry.apache.org/team/new-committers
---
...
| **Committer Account Creation** | The new committer should file the ICLA to the secretary@apache.org, and remember to include the preferred Apache ID on the ICLA form. Here are the guide on [signing the ICLA](/team/sign-icla). The Apache Secretary will help create the Apache ID when processing the ICLA. After the new account is created (will receive *[FORM] Account Request* from root@apache.org), the new committer needs to: (1) [set up the apache.org email account](/team/setup-apache-email-account); (2) complete the setup at [GitBox](https://gitbox.apache.org/setup/) to link the GitHub account for repository write access and enable [GitHub two-factor authentication](https://docs.github.com/en/organizations/keeping-your-organization-secure/managing-two-factor-authentication-for-your-organization/requiring-two-factor-authentication-in-your-organization). In the rare case that the Secretary does not create the account, you may use the legacy template below to request it. | [Request to create a Apache ID (legacy)](#request-to-create-a-new-apache-id-for-new-committer-template-legacy) |
| **Update Roster**              | PPMC members should update the [Roster](https://whimsy.apache.org/roster/ppmc/cloudberry) to add the new committer to the Committer team, so that the appropriate permissions are granted.                                                                                                                                                                                                                                                                                                       |                                                                                                |
...
### Request to create a new Apache ID for new Committer template (legacy)

> **Note:** This template is now a legacy/fallback option. In most cases, the Apache Secretary will create the Apache ID directly when processing the ICLA — there is no need to send this request separately. Only use this template if the Secretary did not create the account after the ICLA has been recorded.
