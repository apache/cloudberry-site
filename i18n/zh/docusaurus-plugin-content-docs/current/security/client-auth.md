...
在 Apache Cloudberry 中，协调器实例的 `pg_hba.conf` 文件用于控制整个系统的客户端访问与认证。虽然各个数据 Segment 也有自己的 `pg_hba.conf` 文件，但它们已经默认配置为仅允许来自协调器主机的连接。由于数据段不接受外部客户端连接，因此无需修改它们的配置文件。
...
默认情况下，`pg_hba.conf` 文件为 gpadmin 用户配置了较宽松的访问权限，但未为其他 Apache Cloudberry 角色启用数据库访问。如果你希望其他用户也能访问数据库，或需要加强 gpadmin 的安全性，就需要手动修改这个配置文件。建议删除使用 `trust` 认证的条目，因为只要能访问服务器的人，就能以任意角色连接数据库。对于本地（UNIX 套接字）连接，推荐使用 `ident` 认证方式，它要求操作系统用户名与连接时指定的角色一致。对于本地和远程的 TCP 连接，`ident` 认证要求客户端主机上运行 ident 服务。你可以在协调器主机上安装 ident 服务，并为本地 TCP 连接（例如 `127.0.0.1/28`）启用 `ident` 认证。不过，对于远程 TCP 连接，`ident` 的安全性较低，因为你必须完全信任客户端主机上的 ident 服务。

:::info
Cloudberry Command Center 提供了图形界面来编辑 `pg_hba.conf` 文件。该界面会在保存前校验配置项，保留文件的历史版本，方便回滚，同时支持将配置重新加载到 Apache Cloudberry 中。
:::

下面是一个示例，展示了如何修改协调器主机上的 `pg_hba.conf` 文件，允许远程客户端以加密密码认证方式访问所有数据库。
...
以下 SSL 服务端文件默认保存在 Apache Cloudberry 协调器的数据目录（`$COORDINATOR_DATA_DIRECTORY`）中：
...
如果启用了协调器主备镜像功能，并使用了 SSL 客户端认证，这些 SSL 文件**不应放在**默认目录 `$COORDINATOR_DATA_DIRECTORY` 中。因为执行 `gpinitstandby` 时，主协调器的数据目录会被整体复制到备协调器中。如果密钥和证书文件为主协调器所用，将导致备协调器启动失败。
...
可以通过设置 `max_connections` 参数来限制系统同时活动的连接数。该参数是本地配置参数，必须分别设置在协调器、备协调器和每个段（主段和镜像段）的 `postgresql.conf` 文件中。建议段实例的 `max_connections` 设置值为协调器的 5 到 10 倍。

设置 `max_connections` 时，还必须配置关联参数 `max_prepared_transactions`。这个值在协调器上必须不小于 `max_connections`，而在段实例上应与协调器一致。

例如：

- 协调器（包含备协调器）中的 `$COORDINATOR_DATA_DIRECTORY/postgresql.conf`：
...
- 所有段实例中的 `SEGMENT_DATA_DIRECTORY/postgresql.conf`：
...
1. 以 Cloudberry 管理员身份登录协调器主机，并执行以下命令加载环境变量：

    ```shell
    source $GPHOME/greenplum_path.sh
    ```

2. 设置 `max_connections` 参数。下面这个命令将在段上设置为 1000，在协调器上设置为 200：

    ```shell
    $ gpconfig -c max_connections -v 1000 -m 200
    ```

    段的连接数必须大于协调器。建议设置为协调器的 5 到 10 倍。

3. 设置 `max_prepared_transactions` 参数。以下命令会在协调器和所有段上统一设置为 200：

    ```shell
    $ gpconfig -c max_prepared_transactions -v 200
    ```

    此参数的值必须大于或等于协调器的 `max_connections` 值。
...
Apache Cloudberry 原生支持客户端与协调器之间建立 SSL 加密连接。SSL 可以防止第三方监听网络数据包，也能避免中间人攻击。只要客户端连接路径不够安全，就应启用 SSL；而当使用客户端证书认证时，则必须启用 SSL。

要启用 Apache Cloudberry 的 SSL 模式，需要满足以下条件：

- 客户端主机与协调器主机（包括备协调器）都必须安装 OpenSSL。
- 协调器与备协调器主机必须正确生成 SSL 文件，包括 `server.key`（服务器私钥）和 `server.crt`（服务器证书）。

    - 私钥文件不应设置口令保护。服务器在启动时不会提示输入私钥口令，如果文件设置了口令，将导致 Apache Cloudberry 启动失败。
    - 在生产环境中，协调器和备协调器主机都应各自使用一对密钥和证书，并在证书的 CN（Common Name） 字段中准确填写各自主机名。

    测试环境可以使用自签名证书，但生产环境应使用由证书颁发机构（CA）签发的证书，以便客户端验证服务器身份。你可以选择全球 CA 或本地 CA。如果所有客户端都在组织内部，推荐使用本地 CA。

- Apache Cloudberry 必须能访问 `server.key` 和 `server.crt`，还要能访问用于认证的其他文件（如 `root.crt`，即受信任 CA 的证书）。在启用 SSL 模式时，协调器会自动查找 `server.key` 和 `server.crt`。默认情况下，如果这些文件不在协调器数据目录（`$COORDINATOR_DATA_DIRECTORY`）中，系统将无法启动。如果你使用了如 `root.crt` 等额外的 SSL 文件，这些文件也必须位于协调器主机上。

    如果启用了协调器主备镜像，并启用了 SSL 客户端认证，SSL 认证相关文件必须同时存在于协调器与备协调器主机上，并且**不应保存在默认目录** `$COORDINATOR_DATA_DIRECTORY`。这是因为 `initstandby` 操作会将协调器的数据目录完整复制到备协调器，若复制了错误的密钥和证书文件（即主协调器的文件而非备协调器自己的文件），将导致备协调器启动失败。

    你可以通过 `postgresql.conf` 中的 `sslcert`、`sslkey`、`sslrootcert` 和 `sslcrl` 参数，将这些文件配置在其他目录。

要启用 SSL，可在协调器和备协调器的 `postgresql.conf` 文件中设置参数 `ssl=on`。例如，使用以下 `gpconfig` 命令开启该参数：
