...
- 客户端与协调器数据库之间的连接可以通过 SSL 加密。该功能由服务器配置参数 `ssl` 控制，默认值为 `off`。将 `ssl` 设置为 `on` 后，客户端与协调器之间的通信将启用加密。需要提前为协调器配置 SSL，具体参考 [OpenSSL 配置](./client-auth.md#openssl-configuration)。

- Apache Cloudberry 支持在 `gpfdist`（Cloudberry 并行文件分发服务）与各段节点之间的网络传输中使用 SSL 加密。参见[加密 gpfdist 连接](#encrypt-gpfdist-connections)。
...
使用 `gpfdists` 时，每个段节点的 `$PGDATA/gpfdists` 目录中必须包含以下客户端证书文件：
...
- 不支持签名功能，意味着不会验证加密子密钥是否归属于协调器密钥；
- 不支持将加密密钥作为协调器密钥，但这种做法本就不推荐，因此不构成实际限制；
